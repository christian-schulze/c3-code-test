...
describe RuleMapping do
  ...
  describe '.map_column_to_rule(column_name, rule_klass, options = {})' do
    it 'instantiates the Exists rule' do
      expect( subject.new('column1', 'Exists').rule.class.name ).to eq 'Rules::Exists'
    end

    it 'instantiates the Integer rule' do
      expect( subject.new('column1', 'Integer').rule.class.name ).to eq 'Rules::Integer'
    end

    it 'instantiates the MinimumLength rule' do
      expect( subject.new('column1', 'MinimumLength', minimum_length: 10).rule.class.name ).to eq 'Rules::MinimumLength'
    end

    it 'instantiates the StateExists rule' do
      expect( subject.new('column1', 'StateExists', default_path: states_fixture_path).rule.class.name ).to eq 'Rules::StateExists'
    end
  end
